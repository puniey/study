...
PyMODINIT_FUNC inituser()
{
	Py_InitModule("user", CPyMethod::m_pyMethods);
}
...
public:
	...
	static std::string Traceback()
	{
		...
		if (pExcValue)
		{
			...
			if (pObjVal)
			{
				const char* errmsg = PyString_AsString(pObjVal);
				Py_DECREF(pObjVal);
				//此处抛出异常直接吞掉
				if (errmsg == NULL)
				{
					...
					PyErr_Clear();
					return "[PythonException]: has exception in CPyExtend::Traceback PyString_AsString";
				}
				strTraceBack += errmsg;
			}
		}
		if (pExcTraceback)
		{
			...
			{
				PyObject* pLine = PyString_FromFormat(", File: \"%s\", line: %d, in \"%s\"",
				...
				if (pLine)
				{
					const char* errmsg = PyString_AsString(pLine);
					Py_DECREF(pLine);
					//此处抛出异常直接吞掉
					if (errmsg == NULL)
					{
...
void func1()
{
	while (true)
	{
		...
		std::cout << CPyExtend::ReloadModule("mytest3") << std::endl;
		std::cout << CPyExtend::ReloadModule("mytest4") << std::endl;
