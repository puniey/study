...
{
    char writebuf[MAXLINE], recvbuf[MAXLINE];
    int n, count;
    while (1)
    {
        memset(writebuf, 0, sizeof(writebuf));
        n = read(STDIN_FILENO, writebuf, sizeof(writebuf));
        if (n > 0 )
        {
             count = write(sockfd, writebuf, n);
        ...
        {
            printf("normal exit, read %d bytes from stdin\n", n);
            break;
        }
